...
void pci_write_word(uint8_t bus, uint8_t slot, uint8_t function, uint8_t offset, uint16_t value){
    ...
    io_out_l(PCI_CONFIG_DATA + (offset & 2), value); //PCIe registers are 4-byte aligned so we need to know whether we are writing to upper two bytes or lower 2 bytes
}
        ...
        for(uint8_t slot = 0; slot < 32; slot++){
            ...
            for(uint8_t func  = 0; func < 8; func++){
                ...
                if(class == class_code && sub_class == subclas_code){
                    ...
                    if((bar0 & 0x06) == 0x04){ //checking value of bits 1 and 2
                        bar1 = pci_read_dword((uint8_t) bus, slot, func, PCI_OFFSET_BAR1);
                    }

                    uint64_t final_address = ((uint64_t)bar1 << 32) | (bar0 & 0xFFFFFFF0); //clearing lower 4 bits of bar0 as they're flags and not address
...
void pci_enable_bus_master(uint8_t bus, uint8_t slot, uint8_t function){
    ...
    if((command & 0x06) != 0x06){ //checking enabled status
        print_address_hex(command);
        command |= 0x06; //enabling it and 1st bit as well which let's cpu read driver's reigisters
        print_address_hex(command);
    }
